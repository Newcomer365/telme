...
async def help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/w s.alert\n"
        "/t t.alert\n"
        f"log: {eth_status}, {sol_status}, {web_status}"
    )

async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip().lower()
    if text == 'e':
        await eth_price(update, context)
    elif text == 's':
        await sol_price(update, context)
    elif text == 'p':
...
def main():
    while True:
        try:
            ...
            app.add_handler(CommandHandler("e", eth_price))
            app.add_handler(CommandHandler("s", sol_price))
